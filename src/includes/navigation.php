<div class="navigation-btn" id="navigation-open">
  <svg version="1.1" id="Layer_1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px"
    viewBox="0 0 512 512" style="enable-background:new 0 0 512 512;" xml:space="preserve">
    <g>
      <g>
  	    <path d="M508.875,493.792L353.089,338.005c32.358-35.927,52.245-83.296,52.245-135.339C405.333,90.917,314.417,0,202.667,0
  		    S0,90.917,0,202.667s90.917,202.667,202.667,202.667c52.043,0,99.411-19.887,135.339-52.245l155.786,155.786
  		    c2.083,2.083,4.813,3.125,7.542,3.125c2.729,0,5.458-1.042,7.542-3.125C513.042,504.708,513.042,497.958,508.875,493.792z
  		    M202.667,384c-99.979,0-181.333-81.344-181.333-181.333S102.688,21.333,202.667,21.333S384,102.677,384,202.667
  		    S302.646,384,202.667,384z"/>
      </g>
    </g>
  </svg>  
</div>

    <nav class="navigation">

      <div class="navigation__left">
        <video id="navigation_video" class="header__video" loop autoplay="autoplay" muted>
          <source src="./src/video/navigation-video.mp4" type='video/mp4; codecs="avc1.42E01E, mp4a.40.2"'>
        </video>
      </div>

      <div class="navigation__right">

        <div class="navigation__close-btn">
          <span class="navigation__close"></span>
        </div>

        <ul class="navigation__list">
          <?php if($_SERVER[REQUEST_URI] == "/gallery") {
            echo "<li class='navigation__list--item'><a href='/'>Home</a></li>";
          } ?> 
          <li class="navigation__list--item"><a href="<?php if($_SERVER[REQUEST_URI] == "/gallery"){ echo "/#aboutMe";} else { echo "#aboutMe";}?>">About Me</a></li>
          <li class="navigation__list--item"><a href="<?php if($_SERVER[REQUEST_URI] == "/gallery"){ echo "/#myCv";} else { echo "#myCv";}?>">My CV</a></li>
          <?php if($_SERVER[REQUEST_URI] !== "/gallery") {
            echo "<li class='navigation__list--item'><a href='./gallery'>Gallery</a></li>";
          } ?> 
          
          <li class="navigation__list--item"><a href="<?php if($_SERVER[REQUEST_URI] == "/gallery"){ echo "/#contactMe";} else { echo "#contactMe";}?>">Contacts</a></li>
        </ul>

        <div class="navigation__socials">
          <a href="https://youtube.com/channel/UCgY8Ju3t4SdcsyGj9wrjUvQ">
            <!--Youtube-->
            <svg viewBox="-21 -117 682.66672 682" xmlns="http://www.w3.org/2000/svg"><path d="m626.8125 64.035156c-7.375-27.417968-28.992188-49.03125-56.40625-56.414062-50.082031-13.703125-250.414062-13.703125-250.414062-13.703125s-200.324219 0-250.40625 13.183593c-26.886719 7.375-49.03125 29.519532-56.40625 56.933594-13.179688 50.078125-13.179688 153.933594-13.179688 153.933594s0 104.378906 13.179688 153.933594c7.382812 27.414062 28.992187 49.027344 56.410156 56.410156 50.605468 13.707031 250.410156 13.707031 250.410156 13.707031s200.324219 0 250.40625-13.183593c27.417969-7.378907 49.03125-28.992188 56.414062-56.40625 13.175782-50.082032 13.175782-153.933594 13.175782-153.933594s.527344-104.382813-13.183594-154.460938zm-370.601562 249.878906v-191.890624l166.585937 95.945312zm0 0"/></svg>
          </a>

          <a href="https://www.instagram.com/nadia.heart/">
            <!--Instagram-->
            <svg viewBox="0 0 512 512" xmlns="http://www.w3.org/2000/svg"><path d="m376 91h-240c-24.8125 0-45 20.1875-45 45v240c0 24.8125 20.1875 45 45 45h240c24.8125 0 45-20.1875 45-45v-240c0-24.8125-20.1875-45-45-45zm-120 270c-57.890625 0-105-47.109375-105-105s47.109375-105 105-105 105 47.109375 105 105-47.109375 105-105 105zm105-180c-16.539062 0-30-13.460938-30-30s13.460938-30 30-30 30 13.460938 30 30-13.460938 30-30 30zm0 0"/><path d="m256 181c-41.351562 0-75 33.648438-75 75s33.648438 75 75 75 75-33.648438 75-75-33.648438-75-75-75zm0 0"/><path d="m436 0h-360c-41.351562 0-76 34.648438-76 76v360c0 41.351562 34.648438 76 76 76h360c41.351562 0 76-34.648438 76-76v-360c0-41.351562-34.648438-76-76-76zm15 376c0 41.351562-33.648438 75-75 75h-240c-41.351562 0-75-33.648438-75-75v-240c0-41.351562 33.648438-75 75-75h240c41.351562 0 75 33.648438 75 75zm0 0"/></svg>
          </a>

          <a href="https://www.facebook.com/nadiaheartartist">
            <!--Facebook-->
            <svg viewBox="0 0 512 512" xmlns="http://www.w3.org/2000/svg"><path d="m475.074219 0h-438.148438c-20.394531 0-36.925781 16.53125-36.925781 36.925781v438.148438c0 20.394531 16.53125 36.925781 36.925781 36.925781h236.574219v-198h-66.5v-77.5h66.5v-57.035156c0-66.140625 40.378906-102.140625 99.378906-102.140625 28.257813 0 52.542969 2.105469 59.621094 3.046875v69.128906h-40.683594c-32.101562 0-38.316406 15.253906-38.316406 37.640625v49.359375h76.75l-10 77.5h-66.75v198h121.574219c20.394531 0 36.925781-16.53125 36.925781-36.925781v-438.148438c0-20.394531-16.53125-36.925781-36.925781-36.925781zm0 0"/></svg>
          </a>
        </div>
      </div>

    </nav>